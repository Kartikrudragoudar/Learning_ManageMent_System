...
class Variant(models.Model):
    ...
    varaint_id  = ShortUUIDField(unique=True, length=6, max_length=20, alphabet="1234567890")
    date = models.DateTimeField(default=timezone.now)
    ...
    def variant_items(self):
        return VaraintItem.objects.filter(variant=self)
...
class VariantItem(models.Model):
    ...
    def __str__(self):
        return f"{self.variant.course.title} - {self.title}"
...
class CartOrder(models.Model):
    ...
    payment_status = models.CharField(choices=PLATFORM_STATUS, default="Processing", max_length=100)
